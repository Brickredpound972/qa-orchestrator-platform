 ...
 * DashboardController
 *
 * Serves the QA Intelligence Dashboard as a single HTML page.
 */
@Controller
@RequestMapping("/qa")
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .badge-risk { display: inline-block; padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: 500; }
  .badge-HIGH { background: #fee2e2; color: #991b1b; }
  .badge-MEDIUM { background: #fef3c7; color: #92400e; }
  .badge-LOW { background: #dcfce7; color: #166534; }
  .badge-release { display: inline-block; padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: 500; }
  ...
  .error { text-align: center; padding: 2rem; color: #dc2626; font-size: 13px; }
...
<div class="container">
  ...
  <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 1.5rem;">
    ...
    <div class="section">
      <div class="section-header">
        <span class="section-title">Release decisions</span>
      </div>
      <div class="chart-wrap">
        <div style="position: relative; height: 200px;">
  ...
  <div class="section">
    <div class="section-header">
      <span class="section-title">Blocked tickets</span>
    </div>
...
function riskClass(level) {
  return 'badge-risk badge-' + (level || 'LOW');
}
function releaseClass(r) {
  return 'badge-release badge-' + (r || 'Go');
}
function fmt(ts) {
  if (!ts) return '-';
  const d = new Date(ts);
...
async function loadSummary() {
  const res = await fetch(BASE + '/qa/api/v1/intelligence/summary');
  ...
  const highCount = d.highRiskCount || 0;
  const totalCount = d.totalAnalyses || 1;
  const medCount = Math.max(0, totalCount - highCount - Math.max(0, totalCount - highCount - Math.floor(totalCount * 0.1)));
  ...
  riskChartInst = new Chart(document.getElementById('riskChart'), {
    ...
    data: {
      ...
      datasets: [{ data: [highCount, medCount, lowCount], backgroundColor: ['#dc2626', '#d97706', '#16a34a'], borderWidth: 0 }]
    },
...
async function loadHistory() {
  const res = await fetch(BASE + '/qa/api/v1/history');
  ...
  let rows = data.map(r => `
    <tr>
      ...
      <td><span class="${riskClass(r.riskLevel)}">${r.riskLevel || '-'}</span></td>
      <td>${r.riskScore ?? '-'}</td>
      <td><span class="${releaseClass(r.releaseRecommendation)}">${r.releaseRecommendation || '-'}</span></td>
  ...
  const releases = data.reduce((acc, r) => { acc[r.releaseRecommendation] = (acc[r.releaseRecommendation] || 0) + 1; return acc; }, {});
  const labels = Object.keys(releases);
  const values = Object.values(releases);
  const colors = labels.map(l => l === 'Block' ? '#dc2626' : l === 'Caution' ? '#d97706' : '#16a34a');
...
async function loadBlocked() {
  const res = await fetch(BASE + '/qa/api/v1/intelligence/blocked');
  const data = await res.json();
...
async function loadAll() {
  try {
    await Promise.all([loadSummary(), loadHistory(), loadBlocked()]);
  } catch(e) {
    console.error(e);
  }
}
